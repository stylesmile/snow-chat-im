...
1. **每个小需求提交一次** - 完成一个小需求立即提交，不积攒
2. **只 commit，不 push** - 除非用户明确要求 `git push`，否则绝不推送
3. **提交身份用 .git/config** - 使用仓库 `.git/config` 中的 `user.name` / `user.email`，不得用 `--author` 覆盖，不得修改 config
4. **提交前测试必须全绿** - 失败的测试不得进入提交
5. **不提交敏感文件** - `.env`、密钥、凭据等不入库
6. **不自动提交** - 仅在用户要求提交或按"每个小需求提交一次"约定时提交
